...
int16_t Dps310::calcHeighMetre(float &heightBuffer, float SeaLevel)
{
    float pressure[20];
    ...
    int16_t ret;
    ret = getContResults(temperature, temperatureCount, pressure, pressureCount);
    if (ret == 0){
        ...
        avg_temperature /= temperatureCount;
        avg_pressure /= pressureCount;

        //heightBuffer = avg_pressure;
        //heightBuffer = avg_temperature;
        heightBuffer = (pow((SeaLevel/avg_pressure), 0.19022)-1)*(avg_temperature+273.15)/0.0065;
    }
    return ret;
